...
void ValidatingLineEdit::onEditingFinished() {
	// use a guard flag to prevent emitting editingFinishedSuccessfully multiple times e.g. when pressed enter
	if (m_editingFinishedEmitted)
		return;
	m_editingFinishedEmitted = true;

	if (isValid())
		emit editingFinishedSuccessfully();

	// Reset guard flag after a short delay, so it's ready for next editing session
	QTimer::singleShot(100, [this]() { m_editingFinishedEmitted = false; });
}
...
void ValidatingLineEdit::onTextChanged ( const QString& ) {
	...
	QPalette basePal;
	pal.setColor(QPalette::Base, basePal.color(QPalette::Base));
	setPalette(pal);

	if (!isEnabled()) {
		setToolTip("");
		setPalette(QPalette()); // reset default palette so that text becomes disabled
		return;
	}
	if (isReadOnly()) {
		QPalette palEdit = palette();
