...
void Table::set(unsigned int rows, unsigned int cols, double width, double outerFrameWidth, double innerFrameWidth,
				QColor backGroundColor) {
	TablePrepare prep;
	prep.m_backGround = backGroundColor;
	prep.m_cols = cols;
	prep.m_rows = rows;
	prep.m_outerFrameWidth = outerFrameWidth;
	prep.m_innerFrameWidth = innerFrameWidth;
	prep.m_width = width;
	set(prep);
}


void Table::init( double outerFrameWidth, double innerFrameWidth, double width,
			   const QStringList &horiHeader, const QStringList & vertiHeader,
			   const std::vector<QStringList> &data,
			   const QColor &color) {
	// size checks


	Q_ASSERT(!data.empty());
	unsigned int rows = (unsigned int)data.size();
	unsigned int cols = (unsigned int)data[0].size();
	bool hasHoriHeader = !horiHeader.empty();
	bool hasVertiHeader = !vertiHeader.empty();
	if(hasHoriHeader) {
		Q_ASSERT(horiHeader.size() == (int)cols);
		++rows;
	}

	if(hasVertiHeader){
		Q_ASSERT(vertiHeader.size() == (int)rows);
		++cols;
	}

	// init
	set(rows, cols, width, outerFrameWidth, innerFrameWidth, Qt::white);
	...
	// set data and format cells
	// set header data
	const unsigned int colAdd = hasVertiHeader ? 1:0;
	const unsigned int rowAdd = hasHoriHeader  ? 1:0;
	if(hasHoriHeader){
		for(unsigned int col = colAdd; col<cols; ++col){
			setCellText(col, 0,boldStart + horiHeader[(int)col - colAdd] + boldEnd);
			cell(col,0).setAlignment(Qt::AlignCenter);
		}
	}


	if(hasVertiHeader){
		for(unsigned int row = rowAdd; row<rows; ++row){
			setCellText(0, row,boldStart + vertiHeader[(int)row - rowAdd] + boldEnd);
			if(row %2 == 1)
				cell(0,row).setBackgroundColor(color);
		}
	}
	...
	for(int col=0; col<data[0].size(); ++col){
		for(unsigned int row=0; row<data.size(); ++row){
			unsigned int c = col + colAdd;
			unsigned int r = row + rowAdd;

			setCellText(c, r, data[row][col]);
			if(r %2 == 1)
				cell(c,r).setBackgroundColor(color);
			cell(c,r).setAlignment(Qt::AlignLeft);
		}
	}
}
		...
		for( unsigned int c=0; c<m_cols; ++c) {
			...
			if( !cell.merged()) {
				...
				if(cell.verticalText()) {
					textSize = cell.textSize(m_rowHeights[(int)r].m_size - cellHeightSpace(cell), m_adaptive);
...
void Table::adjust(QPaintDevice* paintDevice) {
	...
	m_scale = paintDevice->logicalDpiX() / screen->logicalDotsPerInchX();
...
//	qreal maxHeight = maxTextRect.height();
	...
	// m_textDocument->setDefaultStyleSheet("color: black;");
	// qDebug() << "Default style sheet: " << m_textDocument->defaultStyleSheet();
	...
	// MIND: DO NOT REMOVE THIS!
	// This has been added due to the fact, that drawContents() is buggy
	// and the color-palette cannot be changed with setDefaultStylesheet().
	// Why is this important? When whe use a global dark-style in our app, we have
	// a color palette with light colors for all text objects.
	// This palette is inherited in drawContents() and thus the drawn text appears in light grey.
	// To bypass this we habe to manually modify the PaintContext and set a new QPalette with black Text Color.
	if(tooLarge) {
...
std::vector<unsigned int> Table::fittingTableRows(QPaintDevice* , qreal hfirst, qreal hrest) const {
	// makes no sense to draw an empty table
	if( m_rows <= 0 || m_cols <= 0)
...
Table* Table::createSubTable(unsigned int startRow, unsigned int endRow) {
	...
	if(m_headerRows > 0 && startRow <= m_headerRows-1) {
