...
ReportFrameItemConstructionView::ReportFrameItemConstructionView(const ConstructionViewSetup& constructionViewSetup, QPaintDevice* paintDevice,
																 double width, bool horizontal, double spaceAfter, double spaceBefore, bool canPageBreakAfter) :
	ReportFrameItemBase(paintDevice, width, spaceAfter, spaceBefore, canPageBreakAfter),
	m_constructionScene(nullptr)
{
	if(horizontal)
		m_constructionScene = new ConstructionGraphicsSceneHorizontal(false, paintDevice);
	else
		m_constructionScene = new ConstructionGraphicsSceneVertical(false, paintDevice);
	...
	m_constructionScene->setup(m_currentRect.toRect(), paintDevice, constructionViewSetup.m_resolution,
							  constructionViewSetup.m_layers, constructionViewSetup.m_leftBottomLabel, constructionViewSetup.m_rightTopLabel,
