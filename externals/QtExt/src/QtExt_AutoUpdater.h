...
/*! A class that implements all functionality needed for auto-update features.
	...
	* a newer version available (shows the "Update available" dialog. Starts the download of the changelog file and
	...
	\endcode
*/
class QtExt_EXPORT AutoUpdater : public QObject {
	Q_OBJECT
public:
	...
	/*! Call this function from main.cpp to check if a downloaded update installer is available on the specified
		...
		\param installerFilePath Path to installer file.
		\param md5hash MD5 hash of downloaded file, to ensure, that there is no modification to the file before installer
			is started.
		\return Returns true, if a process with the update file was started (in this case the calling program should
			terminate right away).
	*/
	static bool installUpdateWhenAvailable(const QString & installerFilePath, const std::string & md5hash = "");
	...
	/*! Attempts to download the update-info file from a given URL and returns true if
		...
		\param newestRejectedVersion Only applicable when "interactive=false", i.e. when update check is run automatically
			on application start. In interactive mode, the version changelog is always shown.
	...
	*/
	void checkForUpdateInfo(const QString & url, const char * const LONG_VERSION, bool interactive,
	...
	*/
	...
	/*! Change log text (may have [lang::xx] separators and html text). */
	QString					m_changeLogText;
	/*! Change log text, separated by language. */
	QMap<QString, QString>	m_changeLogLanguageText;
