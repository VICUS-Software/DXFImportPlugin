...
void HeaderFrame::update(QPaintDevice* paintDevice, double width) {
	...
	if( !m_userLogo.isNull() )
	{
		...
		m_table.setRowSizeFormat(0, QtExt::CellSizeFormater::Fixed, 200);
	}
...
void HeaderFrame::print(QPainter * p, const QRectF & frame) {
	...
	double scaledWidth = (double)m_userLogo.width() / m_userLogo.height() * 200;
	// double scaledHeight =  * res;

	logoPos.setX( logoPos.x() - scaledWidth);
	logoPos.setY( logoPos.y());
	p->drawPixmap(QRect(logoPos.x(), logoPos.y(), scaledWidth, 200), m_userLogo);
}
