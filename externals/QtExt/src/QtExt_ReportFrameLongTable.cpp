...
void ReportFrameLongTable::update(QPaintDevice* paintDevice, double width) {
	...
	// set table
	m_table.clear();
	m_table.setTableSize(QSize(width, -1));
	configureTable();
...
unsigned int ReportFrameLongTable::numberOfSubFrames(QPaintDevice* paintDevice, double heightFirst, double heightRest) const {
	unsigned int tableCount = (unsigned int)m_table.fittingTableRows(paintDevice, heightFirst - m_headingHeight - 2*m_headingSpace, heightRest).size();
	return tableCount;
}
...
std::vector<QtExt::ReportFrameBase*> ReportFrameLongTable::subFrames(QPaintDevice* paintDevice, double heightFirst, double heightRest) {
	...
	std::vector<QtExt::Table*> subTables = m_table.fittingTables(paintDevice, heightFirst - m_headingHeight - 2*m_headingSpace, heightRest);

	// Q_ASSERT(num == subTables.size());
	// Q_ASSERT(num > 0);
	clearSubFrames();
	m_currentSubFrames.push_back(new ReportFrameTablePart(m_report, m_textDocument, m_heading.clone(), subTables.front(), m_headingSpace));
	for(size_t i=1; i < subTables.size(); ++i) {
		m_currentSubFrames.push_back(new ReportFrameTablePart(m_report, m_textDocument, nullptr, subTables[i], m_headingSpace));
	}
