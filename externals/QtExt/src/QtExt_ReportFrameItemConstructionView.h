...
struct ConstructionViewSetup {
	...
	QString													m_leftBottomLabel;
	QString													m_rightTopLabel;
	int														m_visibleItems = QtExt::ConstructionGraphicsSceneBase::VI_All;
	double													m_height = -1;
	double													m_resolution = 1.0;
	QColor													m_backgroundColor = Qt::white;
	QVector<QtExt::ConstructionGraphicsSceneBase::LineMarker>	m_lineMarker;
	QtExt::ConstructionGraphicsSceneBase::AreaMarker			m_areaMarker;
};

class QtExt_EXPORT ReportFrameItemConstructionView : public QtExt::ReportFrameItemBase
{
public:
	ReportFrameItemConstructionView(const ConstructionViewSetup& constructionView, QPaintDevice* paintDevice, double width, bool horizontal,
									double spaceAfter = 0, double spaceBefore = 0, bool canPageBreakAfter = false);
...
private:
	ConstructionGraphicsSceneBase*	m_constructionScene;
};
