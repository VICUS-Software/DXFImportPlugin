#ifndef QtExt_ValidatingLineEditUnitH
#define QtExt_ValidatingLineEditUnitH
...
/*!	\brief Class ValidatingLineEditUnit provides validation functionality for entered double numbers and
	...
	It has a standard unit which will be shown in the widget.
	...
	\endcode
*/
class QtExt_EXPORT ValidatingLineEditUnit : public QWidget, public ValidatingInputBase
{
	Q_OBJECT

public:
	explicit ValidatingLineEditUnit(QWidget *parent = nullptr);

	~ValidatingLineEditUnit();
	...
	*/
	void setup(const IBK::Unit& unit, double minVal, double maxVal, const QString & toolTip, bool minValueAllowed = false, bool maxValueAllowed = false);
	...
	*/
	void setFromParameter(const IBK::Parameter & p);
	...
	*/
	void setFromParameterOrClear(const IBK::Parameter & p);
	...
	*/
	double value(const IBK::Unit& unit = IBK::Unit()) const;

	/*! Returns content of this widget as IBK::Parameter.
		The current unit is used.
		The parameter is not valid in case the edit is not valid or no unit is set.
	*/
	IBK::Parameter toParameter(const std::string& name);
	...
	*/
	...
	IBK::Unit unit() const { return m_unit; }
...
signals:
	...
	/*! Signal for sending in destructor in order to unregister it from manager (if exist).*/
	void unregister(ValidatingLineEditUnit* myself);

private:
	Ui::ValidatingLineEditUnit *ui;
	IBK::Unit	m_unit;
};


} // namespace QtExt
#endif // QtExt_ValidatingLineEditUnitH
