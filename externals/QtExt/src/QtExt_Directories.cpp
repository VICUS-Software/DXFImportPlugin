...
QString Directories::resourcesRootDir() {
	...
	// for shadow-builds, we expect the working directory to be <repo-root>/bin/debug
#if defined(Q_OS_MAC)
...
#else
	QFileInfo respath(installPath + "/../../" + devdir + "/resources");
	if (!respath.exists())
		respath = QFileInfo(QDir::currentPath() + "/../../" + devdir + "/resources");
	return respath.absoluteFilePath();
#endif
