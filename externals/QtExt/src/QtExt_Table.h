...
class QtExt_EXPORT TablePrepare {
public:
	TablePrepare() :
		m_cols(1),
		m_rows(2),
...
class QtExt_EXPORT Table : public QObject
{
Q_OBJECT
...
public:
	/*! Constructor for use of shared textDocuments.
		...
		\param adaptive If true the table creates at first view a list of all possible cell rects for a fast size change in case of resize.
	...
	*/
	...
	/*! Set the table from given prepare object.
		\param prep Contains all settings (see \a TablePrepare).
	*/
	void set(const TablePrepare& prep);

	/*! Set the table with the given values.
		\param rows Row count
		\param cols Column count
		\param width Width of the complete table
		\param outerFrameWidth Line width for outer frame
		\param innerFrameWidth Line width for inner frame
		\param backGroundColor Background color for the whole table
	*/
	void set(unsigned int rows, unsigned int cols, double width, double outerFrameWidth,
			 double innerFrameWidth, QColor backGroundColor = Qt::white);

	/*! Initialise the table with data.
		\param outerFrameWidth Line width for outer frame
		\param innerFrameWidth Line width for inner frame
		\param width Width of the complete table
		\param horiHeader Contains items for one horizontal header. No header line in case of empty string list.
		\param vertiHeader Contains the items of one vertical header section. No vertical header if empty.
		\param data Contain the data of the table. Size of data vector define row count. Size of first string list define column count.
		\param color Color for alternating row background colors. Whit is fixed for the other one.
	*/
	void init( double outerFrameWidth, double innerFrameWidth, double width,
