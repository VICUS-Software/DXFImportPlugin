...
void FooterFrame::update(QPaintDevice* paintDevice, double width) {
	...
	// show date
	QString dateString = QString("<small>%1</small>").arg(QLocale().toString(QDate::currentDate(), QLocale::ShortFormat));
...
//	p->drawText(tempFrame, m_projectID, opt);
	...
	if ( m_report->m_showPageNumbers ) {
		...
		m_footerTable.setCellText(2,0,pageNumText, Qt::AlignRight);
	}
