...
ConstructionView::ConstructionView(QWidget *parent) :
	...
	m_resolution(1.0),
	m_selectedLayer(-1),
	m_visibleItems(ConstructionGraphicsSceneBase::VI_All),
	m_horizontal(true)
{
...
void ConstructionView::setData(QPaintDevice* paintDevice, const QVector<ConstructionLayer>& layers, double resolution,
							   int visibleItems, bool horizontal) {
	setScene(nullptr);
	delete m_diagramScene;
	m_horizontal = horizontal;
	if (m_horizontal)
		m_diagramScene = new ConstructionGraphicsSceneHorizontal(true, this);
	else
		m_diagramScene = new ConstructionGraphicsSceneVertical(true, this);
	setScene(m_diagramScene);

	connect(m_diagramScene, &ConstructionGraphicsSceneBase::selectionChanged,
			this, &QtExt::ConstructionView::sceneSelectionChanged);
...
void ConstructionView::setAreaMarker(const ConstructionGraphicsSceneBase::AreaMarker& am) {
	if(m_diagramScene == nullptr)
		return;
...
void ConstructionView::updateView() {
	...
	m_diagramScene->setup(frame, m_device, m_resolution, m_inputData, m_leftTopSideLabel, m_rightBottomSideLabel, m_visibleItems);
	show();
...
void ConstructionView::print(QPrinter* printer) {
	...
	int paperWidth = printer->pageLayout().paintRectPixels(printer->resolution()).width();
	...
	std::unique_ptr<ConstructionGraphicsSceneBase> sketch;
	if(m_horizontal)
		sketch.reset(new ConstructionGraphicsSceneHorizontal(true, printer));
	else
		sketch.reset(new ConstructionGraphicsSceneVertical(true, printer));

	sketch->setup(frame, printer, 1.0, m_inputData, m_leftTopSideLabel, m_rightBottomSideLabel, m_visibleItems);
...
QPixmap ConstructionView::createPixmap() {
	...
	std::unique_ptr<ConstructionGraphicsSceneBase> sketch;
	if(m_horizontal)
		sketch.reset(new ConstructionGraphicsSceneHorizontal(true, &pixmap));
	else
		sketch.reset(new ConstructionGraphicsSceneVertical(true, &pixmap));

	sketch->setup(frame, &pixmap, 1.0, m_inputData, m_leftTopSideLabel, m_rightBottomSideLabel, m_visibleItems);
...
void ConstructionView::createSvg(QIODevice * outputDevice) {
	...
	std::unique_ptr<ConstructionGraphicsSceneBase> sketch;
	if(m_horizontal)
		sketch.reset(new ConstructionGraphicsSceneHorizontal(true, &generator));
	else
		sketch.reset(new ConstructionGraphicsSceneVertical(true, &generator));

	sketch->setup(frame, &generator, 1.0, m_inputData, m_leftTopSideLabel, m_rightBottomSideLabel, m_visibleItems);
...
#endif
		QGraphicsItem* item = itemAt(p.toPoint());
