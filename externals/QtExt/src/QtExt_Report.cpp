...
void Report::print(QPrinter * printer, QFont normalFont, Notification *notificationHandler) {
	updateVisibility();
	...
	// Use device pixels for effective page size (consistent with printPage())
	m_effectivePageSize = QSize(printer->pageRect(QPrinter::DevicePixel).width(), printer->pageRect(QPrinter::DevicePixel).height());

	// Calculate global scaling factor/resolution in pix/mm
	m_scale = m_effectivePageSize.width() / double(printer->pageRect(QPrinter::Millimeter).width());  // in pixel/mm
	...
	for (int i=startPage; i<= lastPage; ++i) {
		...
		if (notificationHandler != nullptr)
			notificationHandler->notify((double)i / (double)lastPage);
	}
...
//	const char * const FUNC_ID = "[Report::paintPage]";
	...
	bool skipFirstPage = m_reportSettings->m_skipFooterOnFirstPage && page == 1;
	bool drawHeader = !m_headerFrame.m_isHidden && !skipFirstPage;
	bool drawFooter = !m_footerFrame.m_isHidden && !skipFirstPage;
...
//#define DRAW_DEBUG_FRAMES
		...
		framepen.setStyle(Qt::DashDotLine);
		p->setPen(framepen);
		p->drawRect(m_reportFramesInfos[i].m_rect);
